      ...
      <div className="flex flex-col w-full">
        ...
        {businessUser && (
          <>
            <div className={`relative mb-5 w-full`}>
              <PhoneInput value={phoneNumber} onChange={_OnPhoneNoChange} />
              <PhoneSVG
                classNames={
                  "absolute right-2 top-3 w-6 h-6 pointer-events-none text-gray-500"
                }
              />
            </div>
            <InputField
              name={"website"}
              type={"text"}
              svg={<GlobeSVG classNames={"w-6 h-6 text-gray-500"} />}
        ...
        <div
          className={`floating-input ${
            formik.touched.password && formik.errors.password ? "mb-1" : "mb-4"
          } relative`}
        >
          <input
